...
//https://www.hackerearth.com/challenge/competitive/june-circuits-17/approximate/navi-and-repeated-bracket-subsequences/
namespace CompetitiveProgrammingBracketGeneration
{
    public static class BracketGeneration
    {
        #region Main
        ...
        private static readonly Dictionary<string, List<List<char>>> CacheDictionary = new Dictionary<string, List<List<char>>>();
        private static readonly Dictionary<int, int> PowerDictionary = new Dictionary<int, int>();

        static BracketGeneration()
        {
        ...
        private static void TestCases()
        {
            ...
            for (int i = 0; i < tc; i++)
            {
                ...
                int reqPatternCount = Console.NextInt(true);

                int max = reqPatternCount / 2 + 3;
                var root = (long)Math.Sqrt(reqPatternCount);
                if (root * root == reqPatternCount)
                {
                    s = GetPatternString(root);
                    Console.WriteLine(s);
                    continue;
                }
                ...
                for (int p = 1; p <= max; ++p)
                {
                    ...
                    if (ifNotTested)
                    {
                        ...
                        var toTest = sb.ToString();
                        if (!ShouldPatternBeTested(toTest, reqPatternCount))
                        {
                            continue;
                        }
                        ...
                        var cnt = PatternCountPossible(toTest.ToCharArray());

                        if (cnt == reqPatternCount)
                        {
                    ...
                    long minPatternCount = -1;

                    s = PatternGenerator(pattern.ToCharArray(), reqPatternCount);
                    ...
                    if (minPatternCount > reqPatternCount)
                    {
                        break;
                    }
        ...
        public static string PatternGenerator(char[] seed, int count)
        {

            int len = seed.Length, last = len - 1;
            var baseString = seed;
            baseString[last] = ')';
            var stack = new Stack<char[]>();
            stack.Push(baseString);
            ...
            while (stack.Count > 0)
            {
                ...
                int possibleCount = PatternCountPossible(s);
                ...
                while (l > 0)
                {
                    var newCharArry = new char[s.Length];
                    Array.Copy(s, newCharArry, s.Length);
        ...
        public static int PatternCountPossible(char[] s)
        {
            var key = new string(s);

            if (CacheDictionary.ContainsKey(key))
            {
                return CacheDictionary[key].Count;
            }

            var filteredSubSets = SubSetsOf(s).ToList();

            CacheDictionary.Add(key, filteredSubSets);
            var count = filteredSubSets.Count;

            return count;
        }

        public static bool ShouldPatternBeTested(string s, int count)
        {
            var st = s.ToCharArray();
            int maxCount = PatternCountPossible(st);
        ...
        //PRINT RES
        public static string GetPatternString(long count)
        {
            StringBuilder sb = new StringBuilder();
        ...
        public static string GetPatternString2(long count)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("()");
        ...
        public static IEnumerable<List<char>> SubSetsOf(IEnumerable<char> source)
        {
            var list = source.ToList();
            int length = list.Count;
            int max;
            if (PowerDictionary.ContainsKey(list.Count))
            {
                max = PowerDictionary[list.Count];
            }
            else if (PowerDictionary.ContainsKey(list.Count - 1))
            {
                max = PowerDictionary[list.Count - 1] * 2;
            }
            else
            {
                max = (int)Math.Pow(2, list.Count);
                PowerDictionary.Add(list.Count, max);
            }
            ...
            for (int count = 0; count < max; count++)
            {
                ...
                if (subset.Count == 0 || subset.Count % 2 == 1 || subset[subset.Count - 1] == '(' || subset[0] == ')' || breakApplied)
                {
                    continue;
                }
...
// Submission mode
    ...
    public static class Extensions
    {
        ...
        private static int Partition(long[] arr, int l, int h)
        {
            var x = arr[h];
            int i = (l - 1);
