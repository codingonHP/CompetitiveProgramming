...
namespace Heap
{
    public static class Solution
    {
        ...
        static void Main_Solver(String[] args)
        {
        ...
        private static void Solve()
        {
            ...
            var arry = Console.NextInts(n).OrderBy(c => c).ToArray();

            int count = GetCount1(arry, k);
            Console.WriteLine(count);
        }

        public static int GetCount1(int[] arry, int k)
        {
            arry = arry.OrderBy(c => c).ToArray();
            int n = arry.Length;
            ...
            while (f < heap.Length)
            {
                if (heap[f] >= k) return count;

                var sum = heap[f] + 2 * heap[s];
                ++count;
        ...
        private static void Swim(int[] heap, int f, int pos)
        {
            int parent = (pos - 1 + f) / 2;
            while (parent > f - 1 && heap[pos] < heap[parent])
            {
        ...
        public static int GetCount2(int[] arry, int k)
        {
            int f = 0, s = 1;
            int count = 0;
            ...
            while (f < arry.Length - 1)
            {
                ...
                Array.Sort(arry, f, arry.Length - f);
            }
