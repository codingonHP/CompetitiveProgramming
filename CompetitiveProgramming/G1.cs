...
namespace CpForG1
{
    public static class G1
    {
        ...
        private static void TestCases()
        {
            ...
            var array = Console.NextLongs(tc);
            var sum = Console.NextLong(true);

            long output = GetCount(array, sum);

            Console.WriteLine(output);
        }

        private static long GetCount(long[] array, long sum)
        {
            array = array.OrderBy(c => c).ToArray();
            var subSets = SubSetsOf(array);
            long count = 0;
        ...
        private static long GetGCDCount(long[] array)
        {
            array = array.OrderBy(c => c).ToArray();
            var subSets = SubSetsOf(array);
            long count = 0;
