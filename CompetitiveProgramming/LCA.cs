    ...
    public static class LCA
    {
        ...
        public class Tree
        {
            ...
            public long QueryBf(long n)
            {
                ...
                for (int i = 1; i <= n; i++)
                {
                    for (int j = i; j <= n; j++)
                    {
                        ...
                        var lvalue = Fvalue(stack.Sum(sv => sv.Weight));
                        if (i != j)
                        {
                            lvalue *= 2;
                        }
            ...
            private long GetLValue(int a, int b)
            {
                ...
                var sumAtoB = bigSum - (smallSum - Nodes[smallKey].Value);
                SumDictionary.Add(key, sumAtoB);
