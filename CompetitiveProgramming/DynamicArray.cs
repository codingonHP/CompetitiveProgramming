...
namespace CpForDynamicArray
{
    public static class DynamicArray
    {
        ...
        {
            var input1 = Console.NextInts(2);
            int n = input1[0], q = input1[1];
            int lastAnswer = 0;

            var seqList = new int[n, n];
            var indexArry = new int[n];

            for (int i = 0; i < q; i++)
            {
                var input2 = Console.NextInts(3);
                int qt = input2[0], x = input2[1], y = input2[2];
                int index = (x ^ lastAnswer) % n;

                if (qt == 1)
                {
                    seqList[index, indexArry[index]] = y;
                    indexArry[index] = indexArry[index] + 1;
                }
                else
                {
                    var ymodsize = y % indexArry[index];
                    lastAnswer = seqList[index, ymodsize];
