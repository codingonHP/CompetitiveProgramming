...
namespace CpForCompetitiveProgrammingHRPrimeCellsJuly2017
{
    public static class HRPrimeCellsJuly2017
    {
        #region Main
        ...
        private static void TestCases()
        {
            ...
            int[,] matrix = Console.NextMatrix(tc, tc);
            long count = Solve(matrix);
        ...
        public static long Solve(int[,] matrix)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    ...
                    sum += GetValue(matrix, n, i - 1, j) +
                        GetValue(matrix, n, i + 1, j) +
                        GetValue(matrix, n, i, j - 1) +
                        GetValue(matrix, n, i, j + 1);
        ...
        private static int GetValue(int[,] matrix, int n, int i, int j)
        {
            try
            {
                return matrix[i, j];
