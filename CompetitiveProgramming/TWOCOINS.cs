...
namespace CpForCompetitiveProgrammingTWOCOINS
{
    public static class TWOCOINS
    {
        ...
        public class Tree
        {
            ...
            public readonly Dictionary<long, Node> TreeNodes = new Dictionary<long, Node>();
            public readonly Dictionary<long, Node> Leaves = new Dictionary<long, Node>();

            public void AddNode(long nodeA, long nodeB)
            {
                Node getNodeA, getNodeB;

                if (TreeNodes.ContainsKey(nodeA))
                {
                    getNodeA = TreeNodes[nodeA];
                    Leaves.Remove(nodeA);
                }
                else
                {
                    getNodeA = new Node(nodeA);
                    TreeNodes.Add(nodeA, getNodeA);
                    Leaves.Add(nodeA, getNodeA);
                }

                if (TreeNodes.ContainsKey(nodeB))
                {
                    getNodeB = TreeNodes[nodeB];
                    Leaves.Remove(nodeB);
                }
                else
                {
                    getNodeB = new Node(nodeB);
                    TreeNodes.Add(nodeB, getNodeB);
                    Leaves.Add(nodeB, getNodeB);
                }

                getNodeB.AttachedNodes.Add(nodeA, getNodeA);
                getNodeA.AttachedNodes.Add(nodeB, getNodeB);

                if (Root == null && nodeA == 1)
                {
                    Root = getNodeA;
                    Root.IsRoot = true;
                    getNodeA.IsRoot = true;
                    getNodeB.Parent = getNodeA;
                }
                else if (Root == null && nodeB == 1)
                {
                    Root = getNodeB;
                    Root.IsRoot = true;
                    getNodeB.IsRoot = true;
                    getNodeA.Parent = getNodeB;
                }
        ...
        public class Node
        {
            ...
            public Dictionary<long, Node> AttachedNodes = new Dictionary<long, Node>();
        ...
        private static void TestCases()
        {
            ...
            for (int i = 0; i < tc; i++)
            {
                var nodesCount = Console.NextLong(true);
                Tree tree = new Tree();

                for (int j = 1; j <= nodesCount - 1; j++)
                {
                    var read = Console.NextLongs(2);
                    tree.AddNode(read[0], read[1]);
                }

                var output = Solve(tree);
                Console.WriteLine(output);
            }
        }

#endif
        public static long Solve(Tree tree)
        {
            long refCount = 0;

            foreach (var treeLeaf in tree.Leaves)
            {
                TravelUp(treeLeaf.Value, ref refCount);
            }

            return refCount;
        }

        public static void TravelUp(Node node, ref long count)
        {
            ...
            if (node.AttachedNodes.Count == 1 && parent != null && parent.IsRoot && !parent.IsVisited)
            {
            ...
            TravelUp(grandParent, ref count);
        }
