...
namespace CompetitiveProgramming
{
    public static class Template
    {
        ...
        private static void TestCases()
        {
            ...
            for (long i = 0; i < t; i++)
            {
                long ratio = Console.NextLong();
                long sum = Console.NextLong();
                long prime = Console.NextLong(true);

                long answer = Solve(ratio, sum, prime);
                long answer2 = SolveB(ratio, sum, prime);
                Console.WriteLine(answer + " vs passed : " + answer2);
        ...
        private static long Solve(long ratio, long sum, long prime)
        {
            if (sum == 1)
            {
                return 1;
            ...
            while (true)
            {
                ...
                if (ratio != 1)
                {
                    esum = (Math.Pow(ratio, step) - 1) / d % prime;
                }
                else
                {
                    esum = ratio * step;
        ...
        private static long SolveB(long ratio, long sum, long prime)
        {
            if (sum == 1)
            {
                return 1;
            ...
            while (true)
            {
                ...
                long nextTerm = (currTerm * ratio) % prime;
