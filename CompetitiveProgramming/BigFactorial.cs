...
namespace CpForBigFactorial
{
    public static class BigFactorial
    {
        ...
        public static string Fact(string number, Dictionary<string, string> map)
        {
            ...
            return Multiply(Fact(Dec(number), map), number);
        }
        ...
        public static string Dec(string number)
        {
            if (number.Length == 0) return "";
            if (number.Equals("0")) return "-1";

            char[] c1 = new StringBuilder(number).ToString().Reverse().ToArray();
            ...
            var s = new string(c1.Reverse().Where(c => c != '\0').ToArray());
            return s;
