...
namespace CpForCompetitiveProgrammingTRAVELAL
{
    public static class TRAVELAL
    {
        #region Main
        ...
        private static long Solve(int[,] matrix, int row)
        {
            Dictionary<long, long> dd = new Dictionary<long, long>();
            var dmatrix = DistanceMatrix(matrix, row, out dd);
            var max = (long) -1e9;
        ...
        private static long[,] DistanceMatrix(int[,] matrix, int row, out Dictionary<long, long> dd)
        {
            var distanceMatrix = new long[row, row];
