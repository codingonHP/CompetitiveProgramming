...
namespace CpForCompetitiveProgrammingPSHTTR
{
    public static class PSHTTR
    {
        #region Main
        ...
        public class Path
        {
            ...
            public readonly Dictionary<long, Node> NodeArray;


            public Path(long totalNodes)
            {
                TotalNodes = totalNodes;
                NodeArray = new Dictionary<long, Node>();
            }

            public void AddNode(long from, long to, long cost)
            {
                Node fromNode, toNode;
                if (!NodeArray.ContainsKey(from))
                {
                    fromNode = new Node(from);
                    NodeArray.Add(from, fromNode);
                }
                else
                {
                    fromNode = NodeArray[from];
                }

                if (!NodeArray.ContainsKey(to))
                {
                    toNode = new Node(to);
                    NodeArray.Add(to, toNode);
                }
                else
                {
                    toNode = NodeArray[to];
                ...
                fromNode.Cost[to] = cost;
                fromNode.ReachableNodes.Add(toNode);

                toNode.Cost[from] = cost;
                toNode.ReachableNodes.Add(fromNode);
            }
        }
        ...
        private static void TestCases()
        {
            ...
            for (int i = 0; i < tc; i++)
            {
                long vc = Console.NextLong(true);

                Path path = new Path(vc);
                long u, v;

                for (int j = 0; j < vc - 1; j++)
                {
                    var inputArry = Console.NextLongs(3);
                    u = inputArry[0];
                    v = inputArry[1];
                    long c = inputArry[2];
...
#endif
        public static long Solve(Path path, long u, long v, long k)
        {
            bool reached = false;

            //Testing code.

            //for (int i = 1; i <= path.TotalNodes; i++)
            //{
            //    u = i;
            //    for (int j = 1; j <= path.TotalNodes; j++)
            //    {
            //        v = j;
            //        if (u == v)
            //        {
            ...
            //        Console.Write($"Path: ");
            //        foreach (var n in stk)
            //        {
            //            Console.Write($"{n.NodeNumber} --> ");
            ...
            //            if (n.Cost[prevNode.NodeNumber] <= k)
            //            {
            //                ta ^= n.Cost[prevNode.NodeNumber];
            //                xorStr += n.Cost[prevNode.NodeNumber] + " ^ ";
            //            }
            ...
            //        Console.WriteLine($"");
            //        Console.WriteLine($"XOR Calculation : {xorStr}");
            //        Console.WriteLine($"");
            //        Console.WriteLine($"");
            ...
            foreach (var n in stack)
            {
                ...
                if (n.Cost[prev.NodeNumber] <= k)
                {
                    totalAttractivity ^= n.Cost[prev.NodeNumber];
                }
        ...
        public static void Travel(Path path, long prev, long from, long to, ref bool reached, Stack<Node> stack)
        {
            stack.Push(path.NodeArray[from]);
            var fromNode = path.NodeArray[from];

            if (fromNode.Cost.ContainsKey(to))
            {
                stack.Push(path.NodeArray[to]);
                reached = true;
                return;
            }
            ...
            {
                if (reachableNode.NodeNumber == prev)
                {
                    continue;
                }
