...
namespace CpForKCHAR
{
    public static class KCHAR
    {
        ...
        private static long PrevPow2(long x)
        {
            x = x | (x >> 1);
            x = x | (x >> 2);
            x = x | (x >> 4);
        ...
        public static char GetChar(long n)
        {
            if (n == 0)
            {
                return 'c';
            ...
            if (n % 4 == 3 && IsInteger(Math.Log(n + 1)))
            {
                return 'a';
            }
            ...
            if (n % 4 == 3)
            {
                long prevParent = PrevPow2(n) - 1;
                long mirrorIndex = 2 * prevParent - n;

                var c = GetChar(mirrorIndex + 1);
            ...
            if (n % 4 == 1 && (n / 4) % 2 == 0) return 'a';

            if (n % 4 == 1 && (n / 4) % 2 == 1) return 'c';
        ...
        public static char GetChar2(long k, ref string sc)
        {
            k = k - 1;
            string s = sc;
