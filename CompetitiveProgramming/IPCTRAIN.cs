...
namespace CpForCompetitiveProgrammingIPCTRAIN
{
    public static class IPCTRAIN
    {
        #region Main
        ...
        public class Teacher
        {
            ...
            public long SadnessLevel { get; set; }
        }
        ...
        private static void TestCases()
        {
            ...
            for (int i = 0; i < tc; i++)
            {
                var dataMatrix = new List<Teacher>();
                //var temp = new List<Teacher>();

                var input = Console.NextLongs(2);
                var n = input[0];
                var d = input[1];

                for (int j = 0; j < n; j++)
                {
                    var data = Console.NextInts(3);
                    ...
                    dataMatrix.Add(teacher);
                    //temp.Add(teacher2);
                }


                var output = Solve(n, d, dataMatrix);
                Console.WriteLine(output);

                //Tesing
                //output = SolveBf(n, d, temp);
...
#endif
        public static long SolveBf(long n, long d, List<Teacher> teacherMatrix)
        {
            var data = teacherMatrix.OrderByDescending(p => p.SadnessLevel).ToArray();

            var rem = d;
            var classesAlloted = new int[d + 1];

            foreach (var teacher in data)
            {
                var maxClassAvailableForTeacher = 0;

                for (var i = teacher.ArrivalDay; i < classesAlloted.Length; i++)
                {
                    if (classesAlloted[i] == 0)
                    {
                        maxClassAvailableForTeacher++;
                    }
                }

                if (maxClassAvailableForTeacher > 0 && teacher.ClassCount > 0)
                {
                    var tcc = teacher.ClassCount;

                    if (rem >= maxClassAvailableForTeacher)
                    {
                        teacher.ClassCount -= maxClassAvailableForTeacher;
                    }
                    else
                    {
                        teacher.ClassCount -= rem;
                    }

                    var used = tcc - teacher.ClassCount;
                    ...
                    for (var i = teacher.ArrivalDay; i < classesAlloted.Length && used > 0; i++)
                    {
                        if (classesAlloted[i] == 1)
                        {
                            continue;
                        }
            ...
            long minSadness = 0;
            foreach (var teacher in teacherMatrix)
            {
                minSadness += teacher.SadnessLevel * teacher.ClassCount;
            }

            return minSadness;
        }

        public static long Solve(long n, long d, List<Teacher> teacherMatrix)
        {
            var data = teacherMatrix.OrderByDescending(p => p.SadnessLevel).ToArray();
            SegmentTree tree = new SegmentTree(d);

            Dictionary<long, long> vacancyDictionary = new Dictionary<long, long>();

            var rem = d;
            foreach (var teacher in data)
            {
                if (vacancyDictionary.ContainsKey(teacher.ArrivalDay))
                {
                    continue;
                }

                var maxClassAvailableForTeacher = tree.Query(teacher.ArrivalDay, d, teacher.ClassCount);

                if (maxClassAvailableForTeacher == 0)
                {
                    if (!vacancyDictionary.ContainsKey(teacher.ArrivalDay))
                    {
                        vacancyDictionary.Add(teacher.ArrivalDay, 0);
                    }
                }
        ...
        public class SegmentTree
        {
            ...
            public SegmentTree(long d)
            {
                var count = d;
                int n = (int)Math.Ceiling(Math.Log(count, 10) / Math.Log(2, 10));
                n = (int)Math.Pow(2, n);
                MaxIndex = n;

                Root = CreateTree(n, d, null);
            }

            public long Query(long min, long max, long required)
            {
                return TraverseTree(Root, min, max, ref required);
            }

            private Node CreateTree(long n, long d, Node[] baseNodes)
            {

                var array = new Node[n];

                for (var i = 0; i < array.Length; i++)
                {
                    var left = baseNodes == null ? null : baseNodes[2 * i];
                    var right = baseNodes == null ? null : baseNodes[2 * i + 1];
                    var min = left == null ? i + 1 : left.Min;
                    var max = right == null ? i + 1 : right.Max;
                    var count = left == null ? 1 : left.Data + right.Data;
                ...
                return CreateTree(n / 2, n / 2, array);
            }

            private long TraverseTree(Node node, long min, long max, ref long required)
            {
                int range = 0;
                if (node != null)
                {
                    range = Range(min, max, node.Min, node.Max);
                }

                if (node == null || range == -1 || required == 0 || node.Data == 0)
                {
                    return 0;
                }

                if (range == 1 && node.Left == null)
                {
                    var nc = node.Data;
                    if (nc == 0)
                    {
                ...
                var count = TraverseTree(node.Left, min, max, ref required) + TraverseTree(node.Right, min, max, ref required);
