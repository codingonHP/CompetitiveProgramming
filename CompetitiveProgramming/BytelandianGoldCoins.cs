...
namespace CpForBytelandianGoldCoins
{
    public static class BytelandianGoldCoins
    {
        #region Main
...
#endif
        public static decimal Solve(decimal n, decimal recSum)
        {
            var a = Math.Floor(n / 2);
            var b = Math.Floor(n / 3);
            var c = Math.Floor(n / 4);

            var maxSum = a + b + c;
            decimal maxBreakSum = 0;

            if (maxSum > recSum)
            {
                maxBreakSum += Solve(a, a);
                maxBreakSum += Solve(b, b);
                maxBreakSum += Solve(c, c);
            }
