...
namespace CpForPRATABHI
{
    public static class PRATABHI
    {
        #region Main
        ...
        private static void TestCases()
        {
            ...
            var arry = new int[N + 1];
            var S = new int[N];
            for (int i = 0; i < Q; i++)
            {
                var qt = Console.NextInt();

                if (qt == 1)
                {
                    int index = Console.NextInt();
                    int updateTo = Console.NextInt(true);

                    Update(arry, S, index, updateTo);
                }
                else
                {
                    var xor = Console.NextInt(true);
                    bool xorPossible = XorPossible(xor, S);
                    Console.WriteLine(xorPossible ? "YES" : "NO");
                }
            }

        }

        private static bool XorPossible(int xor, int [] S)
        {
            return Count(S, S.Length, xor) > 0;
        }

        public static void Update(int[] arry, int[] S, int index, int updateTo)
        {
            arry[index] = updateTo;
