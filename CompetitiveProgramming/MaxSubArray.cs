...
namespace CpForMaxSubArray
{
    ...
    public static class MaxSubArray
    {
        ...
        public class Output
        {
            ...
            public long MaxSum { get; set; }
        }

        public static Output MaxValue(int[] array)
        {
            return Divide(array, 0, array.Length - 1);
        }

        private static Output Divide(int[] array, int low, int high)
        {
            if (low >= high)
            {
                return new Output { I = low, J = high, MaxSum = array[low] };
            }

            int mid = (low + high) / 2;

            var maxLeft = Divide(array, low, mid);
            var maxRight = Divide(array, mid + 1, high);

            var maxSum = maxLeft;

            if (maxRight.MaxSum > maxSum.MaxSum)
            {
                maxSum.MaxSum = maxRight.MaxSum;
                maxSum.I = maxRight.I;
                maxSum.J = maxRight.J;
            }
        ...
        public static Output MaxValueBf(int[] array)
        {
